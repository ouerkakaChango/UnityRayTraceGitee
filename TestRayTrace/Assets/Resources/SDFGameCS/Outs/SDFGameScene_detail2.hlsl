...
void SetCheapIndirectColor(inout float3 re, float3 seed, Ray ray, HitInfo minHit, Material_PBR mat)
{
	...
	float p_diffuse = r_diffuse / r_sum;
	float p_specular = r_specular / r_sum;
	//!!! change by me, for a better visual,because pure roughness converge slow with spec, why not make it 1 when r==1
	//(still make posibility sum == 1)
	//p_diffuse += 0.5*mat.roughness;
	//p_specular -= 0.5*mat.roughness;
	//p_diffuse = saturate(p_diffuse);
	//p_specular = saturate(p_specular);
	//___
