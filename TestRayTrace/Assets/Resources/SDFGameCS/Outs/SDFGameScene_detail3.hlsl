...
float3 GetNTanFromR(Texture2D<float3> tex,float2 uv, float3 vtan, int mip = 0)
{
...
float3 SampleNTanFromR(Texture2D<float3> tex,float2 uv, int mip = 0)
{
	uint2 size = GetSize(tex);
	float2 del = 1;//pow(abs(vtan.z),2);
	del/=size;
	float gx = SampleRGB(woodTex, uv+del*float2(1,0),mip).r - SampleRGB(woodTex, uv+del*float2(-1,0),mip).r;
	float gy = SampleRGB(woodTex, uv+del*float2(0,1),mip).r - SampleRGB(woodTex, uv+del*float2(0,-1),mip).r;
	return normalize(float3(gx,gy,0.1));
}
...
else if (inx == 12 )
{
	...
	{
		//???
		//float3 localN = InvRotByEuler(minHit.N, float3(0,0,flipAngle*frac(flipFreq*_Time.y)));
		//uv = SimpleUVFromPos(flipLocalPos, localN, float3(1,1,1));
	}
return uv;
}
...
if(inx == -5)
{
	...
	float k = sp.y/PI; //0:chuizhi,n0,0.5:nAA
	k = clamp(k,0,0.5);
	k = k*2;
	k = pow(k,4);
...
if(inx == 0 )
{
float d = SDFSlice(p, float3(-9.68, 10.24, 0), float3(270, 0, 0), float3(15.561, 15.561, 15.561), slice_scene3_1, 0.015, TraceThre, 0, 0);
re = min(re, d);
}
else if (inx == 1 )
{
...
{
float d = SDFSlice(p, float3(10.16, 10.35, 0), float3(270, 0, 0), float3(15.801, 15.801, 15.801), slice_scene3_2, 0.015, TraceThre, 0, 0);
re = min(re, d);
}
else if (inx == 7 )
{
...
//???
	if(abs(p.x-eyePos.x)<300 && abs(p.z - eyePos.z)<300 )
	{
		float3 grid = float3(5,20,5);
		float3 center = GetGridCenter_DownMode(p,grid,float3(0.0,0.1,5));

		float3 q = p - center;
		float3 bound = 0.5*float3(1,0.015,1);
		//X-Y Plane calculate theta
		float a = (atan2 (q.y, q.x) / PI + 1)*0.5;
		float paperNum = 6;
		...
		for(int i=0;i<paperNum;i++)
		{
			...
			//a_paper += t+iqhash(i+0.2);
			//a_paper += t+iqhash(i+0.2 + 0.001 * center.x);
			a_paper += t+noise(5*i+ center) + 0.2*noise(q);//0.05*fbm4(q);//noise(q);
			float3 paperCenter = float3(CircleToCartesian( a_paper*2*PI,0.5),0);
			if(frac(a_paper)<0.5)
			{
			re = min(re,SDFBox(q,paperCenter,bound, float3(0,0, a_paper*360)));
			}
		}
		re*=0.5;
	}
...
{
//???
	if(abs(p.x-eyePos.x)<300 && abs(p.z - eyePos.z)<300 && traceInfo.lastTrace < 3)
	{
		float3 grid = float3(5,20,5);
		float3 center = GetGridCenter_DownMode(p,grid,float3(0.0,0.1,5));
		...
		float d1 = SDFBox(q-float3(0.5,0,0),0,bound);
		float d2 = SDFBox(q-float3(-0.5,0,0),0,bound);
		re = min(re,d1);
		re = min(re,d2);
	}
}
