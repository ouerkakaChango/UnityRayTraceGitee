...
{
float d = SDFSlice(p, float3(-54.065, 0.378, -56.136), float3(291.8169, 168.0358, 1.048534), float3(1.5, 1.5, 1.5), slice_whiteboardText, 0.03, TraceThre, 0, 0);
re = min(re, d);
}
else if (inx == 2 )
{
...
{
float d = SDFSlice(p, float3(-41.31, 4.45, -53.24), float3(281.6757, 187.4573, 334.7128), float3(2, 2, 2), SDF_q, 0.1, TraceThre, 0, -0.001);
re = min(re, d);
}
else if (inx == 8 )
{
