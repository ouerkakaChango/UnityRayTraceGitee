...
re = min(re, 0 + SDFBox(p, float3(-54.76603, -0.02504051, -56.14224), float3(0.07071168, 1.511707, 0.0646275), float3(338.16, 349.3067, -2.989319E-06)));
	...
	{
		if(abs(p.x)<300 && abs(p.z)<300 && traceInfo.lastTrace<1.5)
		{
			//### grid grass
			float grid = 0.1;
			...
			float2 psign = sign(p.xz - center.xz);

			//### near dir
			float2 c2 = c+grid*psign;
			center = float3(c2.x,CosFBM(c2),c2.y);
			d = SDFGridGrass(p,center);
			re = min(re,d);

			//c2 = c+grid*float2(psign.x,0);
			//center = float3(c2.x,CosFBM(c2),c2.y);
			//d = SDFGridGrass(p,center);
			//re = min(re,d);
			//
			//c2 = c+grid*float2(0,psign.y);
			//center = float3(c2.x,CosFBM(c2),c2.y);
			//d = SDFGridGrass(p,center);
			//re = min(re,d);
