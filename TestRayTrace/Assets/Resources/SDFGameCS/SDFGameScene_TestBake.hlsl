...
if(objInx == 7)
{
GetEnvInfoByID(1,isPNGEnv,envTexArr);
}
	//@@@
}
...
{
	//???
	float3 lightPos = float3(-54.0499992,0.349999994,-56.1599998);
	float3 lightColor = float3(1,1,1);
	//float3 pos = minHit.P;
	//pos.z = 4;
	//pos.x = 3;
...
if(inx == 9||inx == 4||inx == 5||inx == 0||inx == 10||inx == 1)
{
...
else if (inx == 7 && isShow )
{
float3 localp = WorldToLocal(p, float3(-41.31, 4.45, -53.24), float3(281.6757, 187.4573, 334.7128), float3(2, 2, 2));
...
float d = re;
float d2d = re;
float2 picBound = float2(0.5, 0.5) * 2;
float2 p2d = localp.xz * 2;
if (gtor(abs(p2d), picBound))
{
d2d = SDFBox(p2d, 0, picBound) + TraceThre * 2;
d = sqrt(d2d * d2d + dh * dh);
...
float2 uv = p2d / picBound;
uv = (uv + 1) * 0.5;
uint2 picSize = GetSize(SDF_q);
float sdfFromPic = SDF_q.SampleLevel(common_linear_repeat_sampler, uv, 0).r;
sdfFromPic /= picSize.x * 0.5 * sqrt(2) * 2;
sdfFromPic *= picBound.x;
d2d = sdfFromPic;
d2d += 0;
d2d = max(d2d,0);
...
{
re = min(re, 0 + SDFBox(p, float3(-53.56, 1.05, -60.30146), float3(2.5, 2.5, 0.5), float3(0, 0, 0)));
}
else if (inx == 9 )
{
